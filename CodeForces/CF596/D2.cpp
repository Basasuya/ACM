            ...
            for(int j = 1; j <= primeTot; ++j) {
                ...
                if(tmp % Prime[j] == 0) {
                    ...
                    if(ccnt) {
                        ...
                        hash3 = ( (ll)hash3 + (ll)__Pow[id] * ccnt ) % __D;
                        _hash3 = ( (ll)_hash3 + (ll)__Pow[id] * (k - ccnt) ) % __D;
                    }
                }
            }
            if(tmp != 1) {
                ...
                hash3 = ( (ll)hash3 + (ll)__Pow[id] ) % __D;
                _hash3 = ( (ll)_hash3 + (ll)__Pow[id] * (k - 1) ) % __D;
            }
            if(mp.find(make_tuple(_hash1, _hash2, _hash3)) != mp.end())
                ans += mp[make_tuple(_hash1, _hash2, _hash3)];
            mp[make_tuple(hash1, hash2, hash3)] ++;
            // debug(ans);
        }
