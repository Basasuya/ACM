...
inline query(int x1, int y1, int x2, int y2) { // x2 >= x1 && y2 >= y1
    int k = lg[x2 - x1 + 1];
    int l = lg[y2 - y1 + 1];
...
int main() {
    ...
    while(~scanf("%d %d %d", &n, &m, &q)) {
        ...
        auto ok2 = [&](int x1, int y1, int x2, int y2, int len) {
            ...
            // int _k = log2(_x2 - _x1 + 1);
            // int _l = log2(_y2 - _y1 + 1);

            // int tt = max(max(st[_x1][_y1][_k][_l], st[_x1][_y2 - (1<<_l) + 1][_k][_l]), max(st[_x2 - (1<<_k) + 1][_y1][_k][_l], st[_x2 - (1<<_k) + 1][_y2 - (1<<_l) + 1][_k][_l]));
            
            // debug(_x1, _x2, _y1, _y2, tt, _k, _l);
